...
const JOB_STATUSES = ['DRAFT', 'SCHEDULED', 'PUBLISHED', 'CLOSED', 'ARCHIVED']
...
export function AdminTitle({ title, text, action, back }) {
  return <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { sm: 'flex-start' }, gap: 2 }} className="admin-page-title">
...
function Pager({ data, page, setPage }) {
  ...
  return <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center' }} className="table-pager">
    <Typography variant="body2" color="text.secondary">{data.totalElements} records · Page {page + 1} of {data.totalPages}</Typography>
    <Stack direction="row" spacing={1}><Button size="small" disabled={page === 0} onClick={() => setPage((p) => p - 1)}>Previous</Button><Button size="small" disabled={page + 1 >= data.totalPages} onClick={() => setPage((p) => p + 1)}>Next</Button></Stack>
  </Stack>
}
...
const CRUD = {
  ...
  team: {
    ...
    columns: [{ key: 'fullName', label: 'Team member', render: (v, r) => <Stack direction="row" spacing={1.2} sx={{ alignItems: 'center' }}><Avatar src={r.imageUrl}>{v?.[0]}</Avatar><Box><Typography fontWeight={700}>{v}</Typography><Typography variant="caption" color="text.secondary">{r.role}</Typography></Box></Stack> }, { key: 'displayOrder', label: 'Order' }, { key: 'status', label: 'Status', render: (v) => <StatusChip value={v} /> }],
...
export function CrudPage({ resource }) {
  ...
  return <><AdminTitle title={config.title} text={`Create, publish and maintain ${config.title.toLowerCase()}.`} action={<Button size="small" variant="contained" startIcon={<Add />} onClick={() => dedicated ? navigate(`/admin/${resource}/new`) : setEditing({})}>Add new</Button>} />
    {resource === 'jobs' && <JobPublishingPanel />}
...
const RESOURCE_GROUPS = {
  ...
  jobs: [
    ['Role overview', 'Core information candidates see at the top of the role.', ['title', 'slug', 'department', 'location', 'type']],
    ['Publishing schedule', 'Use SCHEDULED + publish date for the daily auto-release queue.', ['status', 'datePosted']],
...
function JobPublishingPanel() {
  const qc = useQueryClient()
  const query = useQuery({ queryKey: ['admin', 'jobs', 'publishing'], queryFn: () => getOne('/admin/jobs/publishing') })
  const [form, setForm] = useState(null)
  useEffect(() => {
    if (query.data?.config) setForm({ ...query.data.config })
  }, [query.data])
  const save = useMutation({
    mutationFn: (values) => api.put('/admin/jobs/publishing', {
      enabled: values.enabled,
      publishPerDay: Number(values.publishPerDay),
      maxOpen: Number(values.maxOpen),
      autoRotate: values.autoRotate,
      publishZone: values.publishZone,
      publishHour: Number(values.publishHour),
    }),
    onSuccess: () => {
  ...
  const run = useMutation({
    mutationFn: () => api.post('/admin/jobs/publishing/run', null, { params: { force: true } }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'jobs'] })
    },
  })
  if (query.isLoading) return <Paper sx={{ p: 3, mb: 3 }}><Skeleton height={180} /></Paper>
  if (query.isError || !form) return <Alert severity="info" sx={{ mb: 3 }}>Job publishing controls are temporarily unavailable.</Alert>
  const counts = query.data.counts || {}
  const queue = query.data.queue || []
  return (
    <Paper className="job-publishing-panel" sx={{ p: 3, mb: 3 }}>
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ justifyContent: 'space-between', alignItems: { md: 'flex-start' }, mb: 2 }}>
        <Box>
          <Typography variant="h5">Auto-publishing</Typography>
          <Typography color="text.secondary" sx={{ mt: 0.5 }}>
            Schedule roles as SCHEDULED with a publish date. The system releases up to N roles each day and can rotate older openings off the public board.
          </Typography>
        </Box>
        <Stack direction="row" spacing={1}>
          <Button size="small" variant="outlined" disabled={run.isPending} onClick={() => run.mutate()}>
            {run.isPending ? 'Publishing…' : 'Publish due now'}
          </Button>
          <Button size="small" variant="contained" disabled={save.isPending} onClick={() => save.mutate(form)}>
            {save.isPending ? 'Saving…' : 'Save schedule'}
          </Button>
        </Stack>
      </Stack>
      <Feedback mutation={save} success="Publishing settings saved." />
      <Feedback mutation={run} success={typeof run.data?.data?.publishedNow === 'number' ? `Released ${run.data.data.publishedNow} role(s).` : 'Publish run completed.'} />
      <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1, mb: 2.5 }}>
        <Chip label={`Open ${counts.published ?? 0}`} size="small" />
        <Chip label={`Queued ${counts.scheduled ?? 0}`} size="small" variant="outlined" />
        <Chip label={`Closed ${counts.closed ?? 0}`} size="small" variant="outlined" />
        <Chip label={query.data.alreadyRanToday ? 'Ran today' : 'Not yet today'} size="small" color={query.data.alreadyRanToday ? 'success' : 'default'} />
      </Stack>
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, minmax(0, 1fr))' }, gap: 2, mb: 3 }}>
        <FormControlLabel control={<Switch checked={!!form.enabled} onChange={(_, checked) => setForm((v) => ({ ...v, enabled: checked }))} />} label="Enable daily auto-publish" />
        <FormControlLabel control={<Switch checked={!!form.autoRotate} onChange={(_, checked) => setForm((v) => ({ ...v, autoRotate: checked }))} />} label="Auto-close oldest when over max open" />
        <TextField label="Publish per day" type="number" value={form.publishPerDay ?? 2} onChange={(e) => setForm((v) => ({ ...v, publishPerDay: e.target.value }))} slotProps={{ htmlInput: { min: 0, max: 20 } }} />
        <TextField label="Max open roles" type="number" value={form.maxOpen ?? 6} onChange={(e) => setForm((v) => ({ ...v, maxOpen: e.target.value }))} slotProps={{ htmlInput: { min: 1, max: 50 } }} />
        <TextField label="Publish hour (0–23)" type="number" value={form.publishHour ?? 9} onChange={(e) => setForm((v) => ({ ...v, publishHour: e.target.value }))} slotProps={{ htmlInput: { min: 0, max: 23 } }} helperText={`Timezone: ${form.publishZone || 'Asia/Hong_Kong'}`} />
        <TextField label="Timezone" value={form.publishZone ?? 'Asia/Hong_Kong'} onChange={(e) => setForm((v) => ({ ...v, publishZone: e.target.value }))} />
      </Box>
      <Typography variant="subtitle2" sx={{ mb: 1 }}>Upcoming queue</Typography>
      {!queue.length ? (
        <Typography color="text.secondary">No SCHEDULED roles in the queue. Create a job, set status to SCHEDULED, and choose a publish date.</Typography>
      ) : (
        <Stack spacing={1}>
          {queue.map((job) => (
            <Stack key={job.id} direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ justifyContent: 'space-between', alignItems: { sm: 'center' }, py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
              <Box>
                <Typography fontWeight={700}>{job.title}</Typography>
                <Typography variant="caption" color="text.secondary">{job.department} · {job.location}</Typography>
              </Box>
              <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
                {job.due && <Chip label="Due" size="small" color="warning" />}
                <Typography variant="body2">{job.datePosted || 'No date'}</Typography>
                <Button size="small" component={Link} to={`/admin/jobs/${job.id}`}>Edit</Button>
              </Stack>
            </Stack>
          ))}
        </Stack>
...
export function ResourceDetailPage({ resource }) {
  ...
  useEffect(() => {
    if (isNew) reset(resource === 'portfolio' ? { featured: false, displayOrder: 0, status: 'DRAFT' } : resource === 'jobs' ? { status: 'SCHEDULED', type: 'FULL_TIME' } : { status: 'DRAFT', type: 'FULL_TIME' })
  ...
  return <Box className="detail-page">
    ...
    <Box component="form" onSubmit={handleSubmit((valuesToSave) => save.mutate(valuesToSave))}>
      <Box className="detail-heading">
        ...
        <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}><StatusChip value={values.status} />{!isNew && <Typography variant="caption" color="text.secondary">Updated {fmtDate(item?.updatedAt || item?.createdAt)}</Typography>}</Stack>
      </Box>
      <Box className="editor-detail-layout">
        ...
        <Stack spacing={2} className="editor-side">
      ...
      <Paper className="detail-action-bar" elevation={0}>
        ...
        <Stack direction="row" spacing={1}>{!isNew && <Button size="small" color="error" onClick={() => setDeleting(true)}>Delete</Button>}<Button size="small" component={Link} to={`/admin/${resource}`}>Cancel</Button><Button size="small" type="submit" variant="contained" disabled={save.isPending}>{save.isPending ? 'Saving…' : isNew ? 'Create record' : 'Save changes'}</Button></Stack>
      </Paper>
...
export function BusinessPlanDetailPage() {
  ...
  return <Box className="detail-page"><DetailBreadcrumbs section="Business Plans" to="/admin/business-plans" current={plan.companyName} />
    <Box className="detail-heading"><Box><Typography variant="overline">Investment application</Typography><Typography variant="h3">{plan.companyName}</Typography><Typography color="text.secondary">{plan.industry || 'Unclassified'} · {plan.stage || 'Stage not specified'} · Submitted {fmtDate(plan.createdAt)}</Typography></Box><Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}><StatusChip value={plan.status} /><Typography variant="caption" color="text.secondary">Step {plan.currentStep || '—'}</Typography></Stack></Box>
    ...
    <Box className="review-layout detail-review-layout"><Stack spacing={2}>
      ...
      <Paper className="detail-section"><Box className="section-heading"><Typography variant="h5">Attached documents</Typography><Typography variant="body2" color="text.secondary">{plan.documents?.length || 0} files submitted for diligence.</Typography></Box>{!plan.documents?.length ? <Typography color="text.secondary">No documents attached.</Typography> : <Stack>{plan.documents.map((doc) => <Stack key={doc.id} direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { sm: 'center' } }} className="document-row"><Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}><Box className="file-icon"><AttachFileOutlined /></Box><Box><Typography fontWeight={700}>{doc.originalName}</Typography><Typography variant="caption" color="text.secondary">{doc.fileType || doc.mimeType || 'Document'} · {fmtSize(doc.fileSize)} · Added {fmtDate(doc.createdAt)}</Typography></Box></Stack><Button size="small" className="detail-inline-action" startIcon={<DownloadOutlined />} onClick={() => downloadFile(`/admin/business-plans/${id}/documents/${doc.id}`, doc.originalName)}>Download</Button></Stack>)}</Stack>}</Paper>
    ...
    <Stack spacing={2}><Paper className="detail-summary sticky-panel"><Typography variant="overline">Review control</Typography><Typography variant="h5">Ownership & decision</Typography><TextField select label="Assigned analyst" value={plan.assignedTo?.id || ''} onChange={(e) => assignMutation.mutate(e.target.value)} disabled={assignMutation.isPending || users.isLoading}><MenuItem value="" disabled>Choose a user</MenuItem>{rowsOf(users.data).map((user) => <MenuItem key={user.id} value={user.id}>{user.firstName} {user.lastName} · {user.email}</MenuItem>)}</TextField><Divider /><TextField select label="Next status" value={decisionStatus} onChange={(e) => setDecisionStatus(e.target.value)} disabled={!transitions.length}><MenuItem value="">Select decision</MenuItem>{transitions.map((value) => <MenuItem key={value} value={value}>{titleCase(value)}</MenuItem>)}</TextField><TextField label="Decision rationale" value={decisionComment} onChange={(e) => setDecisionComment(e.target.value)} multiline rows={4} placeholder="Add context to the review history…" /><Box className="detail-panel-actions"><Button size="small" variant="contained" className="detail-inline-action" startIcon={<Check />} disabled={!decisionStatus || statusMutation.isPending} onClick={() => statusMutation.mutate({ status: decisionStatus, comment: decisionComment })}>{statusMutation.isPending ? 'Recording…' : 'Record decision'}</Button></Box>{!transitions.length && <Alert severity="info">No further status transitions are supported from {titleCase(plan.status)}.</Alert>}</Paper>
...
export function ApplicantDetailPage() {
  ...
  return <Box className="detail-page"><DetailBreadcrumbs section="Applicants" to="/admin/applicants" current={`${person.firstName} ${person.lastName}`} />
    ...
    <Box className="review-layout detail-review-layout"><Stack spacing={2}>
      ...
      <Paper className="detail-section"><Box className="section-heading"><Typography variant="h5">Resume & cover letter</Typography><Typography variant="body2" color="text.secondary">Original application files retained by the recruitment API.</Typography></Box><Stack spacing={1}>{person.resumeName && <Box className="material-card"><Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}><Box className="file-icon"><AttachFileOutlined /></Box><Box><Typography fontWeight={700}>{person.resumeName}</Typography><Typography variant="caption" color="text.secondary">Candidate resume</Typography></Box></Stack><Button size="small" className="detail-inline-action" startIcon={<DownloadOutlined />} onClick={() => downloadFile(`/admin/applicants/${id}/resume`, person.resumeName)}>Download</Button></Box>}{person.coverLetterName && <Box className="material-card"><Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}><Box className="file-icon"><AttachFileOutlined /></Box><Box><Typography fontWeight={700}>{person.coverLetterName}</Typography><Typography variant="caption" color="text.secondary">Cover letter attachment</Typography></Box></Stack><Button size="small" className="detail-inline-action" startIcon={<DownloadOutlined />} onClick={() => downloadFile(`/admin/applicants/${id}/cover-letter`, person.coverLetterName)}>Download</Button></Box>}</Stack>{person.coverLetter && <><Divider /><Typography variant="subtitle2">Cover letter response</Typography><Typography className="rich-text compact-copy">{person.coverLetter}</Typography></>}</Paper>
...
export function SettingsPage() {
  ...
  const mutation = useMutation({ mutationFn: () => Promise.all((query.data || []).filter((setting) => String(values[setting.key] ?? '') !== String(setting.value ?? '')).map((setting) => api.put(`/admin/settings/${encodeURIComponent(setting.key)}`, { key: setting.key, value: String(values[setting.key] ?? ''), description: setting.description, category: setting.category }))), onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin', 'settings'] }); qc.invalidateQueries({ queryKey: ['admin', 'jobs', 'publishing'] }) } })
  const categories = useMemo(() => [...new Set((query.data || []).map((x) => x.category).filter(Boolean))], [query.data])
  return <><AdminTitle title="Settings" text="Edit runtime configuration. Careers auto-publish keys also appear under Job Management." /><Feedback mutation={mutation} success="Settings saved." /><Paper className="settings-form"><TextField select label="Category" value={category} onChange={(e) => setCategory(e.target.value)}><MenuItem value="">All categories</MenuItem>{categories.map((x) => <MenuItem key={x} value={x}>{titleCase(x)}</MenuItem>)}</TextField>{query.isLoading ? <Skeleton height={250} /> : query.isError ? <Alert severity="error">{errorText(query.error)}</Alert> : !query.data?.length ? <Typography color="text.secondary">No settings in this category.</Typography> : <Stack spacing={2}>{query.data.map((setting) => <TextField key={setting.key} label={setting.description || setting.key} value={values[setting.key] ?? ''} onChange={(e) => setValues((v) => ({ ...v, [setting.key]: e.target.value }))} helperText={`${setting.key}${setting.category ? ` · ${setting.category}` : ''}`} />)}<Button variant="contained" disabled={mutation.isPending} onClick={() => mutation.mutate()} sx={{ alignSelf: 'flex-start' }}>{mutation.isPending ? 'Saving…' : 'Save changed settings'}</Button></Stack>}</Paper></>
}
